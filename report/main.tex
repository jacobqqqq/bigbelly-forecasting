...
\begin{titlepage}
  ...
  \vspace{0.4cm}
  {\normalsize IND ENG 180 - Industrial Engineering Capstone, Project 10A\par}
  \vspace{0.4cm}
  {\normalsize University of California, Berkeley\par}
  \vspace{0.4cm}
  {\normalsize March 20, 2026\par}
  \vspace{0.6cm}
  {\normalsize\color{gray}\textit{DRAFT - First Half of Final Report}\par}
\end{titlepage}
...
% ── DRAFT NOTES ───────────────────────────────────────────────────────────────
\section*{Draft Notes and Pending Work}
\addcontentsline{toc}{section}{Draft Notes and Pending Work}

This document is a draft of the first half of the final report. The following items are
in progress and will be incorporated before final submission.

\begin{itemize}

  \item \textbf{Wasted trip figures.} Three-panel visualization (category breakdown,
  by-stream breakdown, quarterly trend) and the top-15 bins by unexplained wasted trips
  will be added as numbered figures in the EDA section.


  \item \textbf{Recency correction.} A correction mechanism that adjusts fill-curve
  projections based on recent deviations from the fitted curve is planned but not yet
  implemented.

  \item \textbf{Integration with Project 10B.} Handoff format, required fields, and
  update cadence are pending coordination with Team 10B.

  \item \textbf{Sustainable implementation recommendations.} A recommendations section
  covering retraining cadence, CLEAN export process, staff interface design, and
  procedures for handling operational changes will be included in the final report.

  \item \textbf{Final model selection.} The final report will recommend a specific
  approach for deployment based on completed evaluation.
